...
## Box Plot <small>Box-and-Whisker Plot</small>

A single numeric variable is sorted and then split into **quarters**. The *central* 50% of the variable is visualised as a box and the *remainder*[\*]{.fragment fragment-index=2 .highlight-red} as lines
...
plotObj + 
  ...
  labs(y = NULL) + 
  scale_y_continuous(breaks = NULL) +
  geom_vline(xintercept = c(boxplot.stats$xmin_final, boxplot.stats$xlower, boxplot.stats$xmiddle, boxplot.stats$xupper, boxplot.stats$xmax_final), linetype = 5, linewidth = 1, colour = "darkblue")
```
:::
...
plotObj + 
  geom_boxplot(fill = "lightblue", outlier.stroke = 1.5, outlier.shape = 1, outlier.size = 4.5) + 
  labs(y = NULL) +
...
[\*]{.red}Most software uses the spread of the *central* 50% of the variable to identify "interesting" observations
:::
