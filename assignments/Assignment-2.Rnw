...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Tom wants to model the average time spent waiting in queue for a sample of 49 customers. State the approximate probability distribution for the random variable, $\overline{Y}$, and its parameters to model this scenario.
