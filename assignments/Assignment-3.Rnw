...
\pagestyle{fancy}
\fancyhf{}
\lhead{\textcolor{hfgray}{Assignment Three}}
...
\section{ENGEN102 -- Assignment Three}

\textbf{Due 11 am Monday, 31st July}
...
\begin{enumerate}
  ...
  \item {\small \color{Orange} \sffamily [C marks]} Arsenic is a known carcinogen and poison. The standard laboratory procedures for measuring arsenic concentration ($\mu$g/L) in water are expensive.

The following numeric summaries were calculated from data that compared the standard procedure to a new inexpensive field method.
\begin{equation*}
\bar{x}_{Standard} = 19.7 ~ \mu\text{g/L}, \quad s_{Standard} = 1.1 ~ \mu\text{g/L}, \quad n_{Standard} = 10
\end{equation*}
\begin{equation*}
\bar{x}_{New} = 10.9 ~ \mu\text{g/L}, \quad s_{New} = 0.6 ~ \mu\text{g/L}, \quad n_{New} = 10
\end{equation*}

Construct a 95\% confidence interval for $\mu_{Standard} - \mu_{New}$.
\end{enumerate}

\subsection{Tutorial problems (not for handing in)}
\medskip

You can answer \textbf{Questions A} and \textbf{B} as if the assumptions for inference were met.
...
<<include=FALSE>>=
library(s20x); library(iNZightPlots)
data(peru.df)
pdf(file = "Peru.pdf", width = 7, height = 4.5, onefile = FALSE)
iNZightPlot(BP, data = peru.df, box.fill = "white", main = "Distribution of Blood Pressure Measurements", xlab = "Systolic Blood Pressure (mm Hg)")
dev.off()
@
  ...
  \begin{enumerate}
    \item Describe \textit{at least} two features of the systolic blood pressure measurements for this sample of Peruvian Indians.
    \item A research question for this data was to assess the long-term effects of altitude on blood pressure. Normal systolic blood pressure is around 120 mm Hg or lower\footnote{https://www.stroke.org.nz/blood-pressure}. Based on this data, is it plausible that living at a high altitude increases systolic blood pressure? Explain your answer.
  \end{enumerate}
\end{enumerate}
...
\begin{itemize}
  \item Practice Exercise 308:\ Questions 5 and 6
  \item Practice Exercise 309:\ Questions 1--3
  \item Practice Exercise 311:\ Questions 2, 4, 5, and 6
  \item Practice Exercise 312:\ Questions 1, 2, 3, 5, and 6
\end{itemize}
