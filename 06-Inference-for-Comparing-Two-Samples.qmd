...
## Standard error for *x&#772;*~1~ &minus; *x&#772;*~2~ {.margin-bottom}

Recall that a standard error describes how precise a statistic is as an estimate

. . .

One approach is to assume that the observed data comes from two separate "random" processes^[Independence between groups], and *within* process: observations are independent and identically distributed^[Independence within group]

. . .

Hence this definition for $\text{se}(\bar{x}_1 - \bar{x}_2)$ comes using the Central Limit Theorem to study the random variable, $D = \overline{X}_1 - \overline{X}_2$, under the above assumptions:
