...
Hence a random variable *models* the outcomes we could potentially observe from a random process[, and a potential **sample** of the random process]{.fragment}
...
Let $X$ be a **continuous** random variable that follows the Normal distribution, that is,
...
The function a Normally distributed **continuous** random variable uses is
...
:::: {.fragment}
For [Ex 3]{style="color: tomato;"}, [iNZight]{.R} calculates that $\bar{x} = 5.49 ~ (2 ~\text{dp})$ and $s = 0.06 ~ (2 ~\text{dp})$
::::
...
Since a random variable that is Normally distributed is a **continuous** random variable, we assign probabilities to intervals

. . .

Suppose $X \sim \text{Normal}(\mu, \sigma)$

::: {.incremental .tightList}
- $\mathbb{P}(a < X < b) > 0$
- $\mathbb{P}(X = x) = 0$
:::
...
## Calculating probabilities by hand
